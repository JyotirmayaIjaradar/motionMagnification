...
#  motion magnification  using butterwoth filter
def videoMagnificationButterWorthFilter(fileToProcess, alpha, lambda_c, samplingRate, chromeAttenuation, lowFreq, highFreq):
    ...
    ## save frames to local storage as well
...
# Motion magnification using ideal filter
def videoMagnificationIdealFilter(fileToProcess, alpha, samplingRate, chromeAttenuation, lowFreq, highFreq):
    # Select the source the and result directories
    datadir = os.getcwd()
    ...
    success, image = videoInputFn.read()
    frameIndex = 1
    print("Processing the input video", fileToProcess , "...")
    print("Extracting original video frames...")
    while success:
        frames[frameIndex-1,:,:,:] = np.float32(image/255.0)
        cv2.imwrite(vidFramesFn + "/frame%04d.png" %(frameIndex), image)
