% function for calculating phase based motion magnification.
% take the input of magnification factor, frequency band sigma etc.

function outName = phaseAmplify(vidFile, magPhase , fl, fh,fs, outDir, varargin)

    %% Read Video the input video
    ...
    %% Calculate the spatial filter for the video       
    ...
    %% Initialization of motion magnified component
    ...
    %% Now first calculate phase differences from reference frame
    ...
    % loop fo culating the phase for each pyramid level
    for level = 2:numLevels-1
        %% Calculate phases for each level
        ...
        %% Temporal Filtering of phases
        fprintf('Applying bandpassing filter to the phases\n');
        delta = temporalFilter(delta, fl/fs,fh/fs); 


        %% Apply the magnification

        fprintf('Applying magnification to the phases\n');
        for frameIDX = 1:nF

            phaseOfFrame = delta(:,:,frameIDX);
            originalLevel = buildLevel(vidFFT(:,:,frameIDX),level);
            %% Amplitude Weighted Blur for sigma = 0        
            if (sigma~= 0)
...
%define the output file name based on the given parameters
